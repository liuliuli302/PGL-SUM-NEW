...
if __name__ == "__main__":
    ...
    for split_id in range(5):
        # Model data
        model_path = f"../PGL-SUM-Modify/inference/pretrained_models/table{table}_models/{dataset}/split{split_id}"
        model_file = [f for f in listdir(model_path) if isfile(join(model_path, f))]

        # Read current split
        split_file = f"../PGL-SUM-Modify/data/splits/{dataset.lower()}_splits.json"
        with open(split_file) as f:
            data = json.loads(f.read())
            test_keys = data[split_id]["test_keys"]

        # Dataset path
        dataset_path = f"../PGL-SUM-Modify/data/{dataset}/eccv16_dataset_{dataset.lower()}_google_pool5.h5"
